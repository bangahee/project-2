...
- `Quiz`: 문제, 선택지, 정답을 관리하는 클래스
- `QuizGame`: 메뉴, 입력 처리, 게임 진행, 파일 저장/불러오기 등 전체 흐름을 관리

→ 데이터와 기능을 클래스 단위로 분리하여 구조를 명확하게 설계했습니다.

---

### 로직 분리 기준

- 입력 처리: `get_number_input`, `get_text_input`
- 게임 진행: `play_quiz`, `add_quiz`, `list_quizzes`
- 데이터 처리: `load_state`, `save_state`

→ 역할이 다른 기능을 분리하여 가독성과 재사용성을 높였습니다.
...
→ 프로그램 시작 시 데이터를 불러오고, 변경 시마다 저장하도록 설계했습니다.
...
- JSON 파일 오류 발생 시 기본 데이터로 복구

→ 프로그램이 중단되지 않고 안정적으로 동작하도록 설계했습니다.
...
- 기능 단위로 커밋 수행
- `Init`, `Feat`, `Refactor`, `Docs`, `Test` 규칙 사용
- 브랜치 분리 후 merge로 기능 통합

→ git log를 통해 merge commit을 확인했습니다.
...
→ 데이터와 기능을 하나로 묶어 관리하기 위해 사용했습니다.  
→ 함수만 사용할 경우보다 구조가 명확하고 유지보수가 용이합니다.
...
- 프로그램 종료 후에도 데이터 유지
- Python dict 구조와 유사하여 변환이 용이

```

Quiz 객체 → dict → JSON 저장 → JSON 읽기 → 객체 복원
...
- 파일 없음, JSON 손상, 입력 오류 등 다양한 실패 상황 처리
→ 프로그램이 비정상 종료되지 않도록 하기 위해 사용했습니다.
...
→ 기능을 독립적으로 개발하고 안정적으로 통합할 수 있습니다.
...
→ 서로 다른 데이터(목록 / 단일 값)를 분리하여 관리하도록 설계했습니다.

---

## 확장 및 개선 고려

* 데이터가 많아질 경우 JSON 방식은 성능 저하 가능 → DB 사용 고려
* JSON 손상 시 기본 데이터로 복구 및 재저장
* 기능 변경 시 해당 클래스/메서드 단위로 수정 가능
