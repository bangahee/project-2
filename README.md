...
프로젝트를 진행하면서 클래스 설계, 데이터 저장 방식, 예외 처리, Git workflow 등 핵심 개념을 정리했습니다.

### 1. 클래스 구조 설계

- `Quiz` 클래스: 문제, 선택지, 정답을 관리
- `QuizGame` 클래스: 전체 흐름(메뉴, 입력, 게임 진행, 저장)을 관리

→ 클래스는 데이터와 기능을 하나로 묶어 관리할 수 있어 구조가 명확해지고 유지보수가 쉬워집니다.

---

### 2. 입력 처리 및 흐름 분리

- 입력 처리, 게임 진행, 데이터 저장을 각각 분리하여 구현
- 입력은 별도의 함수로 처리 (`get_number_input`, `get_text_input`)

→ 입력 오류를 한 곳에서 처리하고, 동일한 검증 로직을 재사용하기 위해 분리했습니다.

---

### 3. JSON 저장 구조

- 퀴즈 데이터를 `dict` 형태로 변환 후 JSON으로 저장
...
Quiz 객체 → dict → JSON 저장 → JSON 읽기 → dict → 객체 복원

````

→ 객체는 직접 저장할 수 없기 때문에 dict로 변환 후 JSON으로 저장하도록 설계했습니다.
...
→ 서로 다른 성격의 데이터를 분리하여 관리하도록 설계했습니다.
→ 이후 통계, 사용자 정보 등 확장에도 유리합니다.
...
→ 프로그램이 중간에 종료되지 않도록 안정성을 확보했습니다.
...
→ 사용자 데이터 손실을 방지하고, 항상 실행 가능한 상태를 유지하도록 설계했습니다.
...
* 기능 단위로 커밋 수행
* `Init`, `Feat`, `Refactor`, `Docs`, `Test` 형태 사용

→ 커밋 로그만 보고도 작업 흐름과 변경 내용을 쉽게 파악할 수 있도록 했습니다.
...
* 기능 개발을 위해 브랜치 사용
* merge를 통해 main 브랜치에 통합

→ 기능을 독립적으로 개발한 뒤 하나의 프로그램으로 합칠 수 있도록 했습니다.

→ 실제로 브랜치 생성, commit, merge, 충돌 해결 과정을 수행했습니다.
...
→ 다양한 상황에서도 프로그램이 안정적으로 동작하도록 설계했습니다.

---

### 10. JSON 방식의 한계 및 개선 아이디어

* 퀴즈 데이터가 1000개 이상으로 증가할 경우:
...
* JSON 파싱 실패 시 기본 데이터로 복구
* 이후 정상 파일로 재저장

→ 사용자 데이터가 완전히 사라지지 않도록 보호

---

### 12. 기능 변경 시 구조 대응

* 기능 변경 시 해당 클래스 또는 메서드 단위로 수정

예:

* 점수 계산 변경 → `play_quiz()` 수정
* 문제 구조 변경 → `Quiz` 클래스 수정
* 입력 방식 변경 → 입력 처리 함수 수정

→ 기능별로 코드가 분리되어 있어 수정이 용이함
